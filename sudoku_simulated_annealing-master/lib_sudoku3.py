...
class Sudoku(object):
    ...
    def __init__(self, input_file, seed=0):
        ...
        self._fill_puzzle()
        print(self.puzzle.min())
        assert self.puzzle.min() >= 1
        assert self.puzzle.max() <= 1
    ...
    def _load_puzzle(self, filename):
        ...
        temp=x.shape
        print(temp)
        assert x.shape == (3, 3), 'ERROR: wrong-shape puzzle in %s' % filename

        #assert x.max() <= 3, 'ERROR: puzzle.max() = %i' % x.max()
    ...
    def _get_box(self, i, j):
        ''' Returns entries of the 2x4 box for the cell (i, j).
        '''
        print(self.puzzle[(i // 3) * 1:(i // 3 + 1) * 1,
                           (j //3 ) * 1:(j // 3 + 1) * 1])
        return self.puzzle[(i // 3) * 1:(i // 3 + 1) * 1,
                           (j //3 ) * 1:(j // 3 + 1) * 1]
