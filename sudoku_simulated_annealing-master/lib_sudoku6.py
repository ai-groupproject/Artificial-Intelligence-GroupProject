...
class Sudoku(object):
    ...
    def _load_puzzle(self, filename):
        ...
        temp=x.shape
        print(temp)
        assert x.shape == (6, 6), 'ERROR: wrong-shape puzzle in %s' % filename

        assert x.max() <= 6, 'ERROR: puzzle.max() = %i' % x.max()
    ...
    def _get_box(self, i, j):
        ...
        return self.puzzle[(i // 2) * 2:(i // 2 + 1) * 2,
                           (j // 4 ) * 4:(j // 4 + 1) * 4]
